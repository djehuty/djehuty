...
public:
   ...
   */
  this(bool delegate(ubyte[] buffer) readInto,
   ...
   */
  void readInto(bool delegate(ubyte[] length) f);
   ...
   *  not enough bytes in the stream, the function will do nothing and return
   *  false.
   * buffer: A buffer that will receive the read data.
   * Returns: true when the buffer can be filled. false otherwise, and the
   *  buffer remains untouched and the stream unchanged.
   */
   ...
   * Returns: A byte array containing up to the number of bytes requested.
   */
